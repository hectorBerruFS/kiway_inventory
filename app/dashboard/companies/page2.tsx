...
function CompanyDialog({
  ...
  const [monthlyBudget, setMonthlyBudget] = useState(company?.monthlyBudget || "");
  const [supervisorId, setSupervisorId] = useState(company?.supervisorId || "");

  if (open && company && name !== company.name) {
      ...
      <DialogContent className="max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <Button type="submit" disabled={loading} className="h-11">
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : company ? "Guardar Cambios" : "Crear Empresa"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
